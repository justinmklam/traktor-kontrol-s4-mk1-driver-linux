...
EvDevHelper::EvDevHelper(ConfigHelper *config) {
  ...
  if (traktor_device_id_ == -1) {
    logger->error("[EvDevHelper:EvDevHelper] {0} Device not found.... Bye!",
...
void EvDevHelper::check_evdev_status() {
  ...
  if (rc < 0) {
    logger->error(
        "[EvDevHelper::check_evdev_status] Failed to init evdev device: {0}",
        strerror(-rc));
    exit(EXIT_FAILURE);
  }
  ...
  do {
    ...
    logger->debug("[EvDevHelper::check_evdev_status] Check evdev Status: {0}",
                  rc);

    usleep(this->config_helper->get_int_value(
...
void EvDevHelper::read_events_from_device(RtMidiOut *pMidiOut) {
  ...
  if (rc < 0) {
    logger->error(
        "[EvDevHelper::read_events_from_device] Failed to init libevdev ({0})",
        strerror(-rc));
    exit(EXIT_FAILURE);
  }
  const int fd = libevdev_get_fd(dev);
  ...
  while (true) {
    ...
    if ((pfd.revents & POLLERR) || (pfd.revents & POLLHUP) ||
      ...
      logger->error(
          "[EvDevHelper::read_events_from_device] evdev device disconnected or "
          "became invalid: {0}",
          pfd.revents);
      break;
    }
    ...
    while (true) {
      ...
      if (rc == LIBEVDEV_READ_STATUS_SUCCESS) {
        ...
        if ((ev.code ==
          ...
          if (toggle_ac) {
            ...
            AlsaHelper::bulk_led_value(traktor_device_id_, to_on, Led::ON, 4,
                                       config_helper);
            AlsaHelper::bulk_led_value(traktor_device_id_, to_low, Led::MIDDLE,
                                       1, config_helper);
            AlsaHelper::bulk_led_value(traktor_device_id_, to_off, Led::OFF, 4,
                                       config_helper);
          }
          logger->debug(
              "[EvDevHelper::read_events_from_device] Deck toggle AC Changed: "
              "{0}",
              toggle_ac);
          continue;
        }
        if ((ev.code ==
          ...
          logger->debug(
              "[EvDevHelper::read_events_from_device] Deck toggle BD Changed: "
              "{0}",
              toggle_bd);
          continue;
        }
        ...
        if ((type_name != nullptr) && (code_name != nullptr)) {
          logger->debug(
              ...
              "CodeName: {1} - Type: {2} - Code: {3} - Value: {4} - Time: {5}",
              type_name, code_name, ev.type, ev.code, ev.value, ev.time.tv_sec);
        } else {
          logger->debug(
              "[EvDevHelper::read_events_from_device] Event: Type: {0} - Code: "
              "{1} - Value: {2} - Time: {3}",
        ...
        evdev_event.handle_with(pMidiOut, traktor_device_id_, shift_ch1,
                                shift_ch2, toggle_ac, toggle_bd, config_helper);
      }
    }
  }
