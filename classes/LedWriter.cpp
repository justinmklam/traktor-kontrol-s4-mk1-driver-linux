...
void LedWriter::worker_loop() {
  while (true) {
    std::map<int, int> updates;
    {
      ...
      cv_.wait(lock, [this] { return !running_ || !pending_updates_.empty(); });
...
bool LedWriter::write_led(int control_id, int value) {
  ...
  int result = snd_ctl_elem_write(control_, control_value);
  if (result == -ENODEV || result == -ENXIO) {
    logger_->info(
        "[LedWriter::write_led] ALSA controller disconnected; "
        "reopening LED control");
    close_control();
    card_id_ = AlsaHelper::get_traktor_device(config_helper_);
    std::fill(std::begin(shadow_state_), std::end(shadow_state_), -1);
    if (card_id_ != -1 && open_control()) {
      result = snd_ctl_elem_write(control_, control_value);
    }
  }
